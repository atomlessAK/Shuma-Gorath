...
If `SHUMA_API_KEY` is missing, `/admin/*` endpoints are disabled. The insecure placeholder key (`changeme-supersecret`) is rejected unless `SHUMA_DEBUG_HEADERS=true` (dev/debug mode).
