...
- [x] H4.4.2 slice completed: added explicit `external` provider adapters in `src/providers/external.rs`; `fingerprint_signal` now routes to an external stub contract, while `ban_store`, `challenge_engine`, and `maze_tarpit` remain explicit unsupported adapter paths with safe fallback semantics and ban-sync failure signaling.
...
- [x] H4.6.1 slice completed: external `rate_limiter` now uses Redis-backed distributed counting (`INCR` + TTL) with explicit internal fallback and provider implementation labeling (`external_redis_with_internal_fallback`); `ban_store` external sync semantics remain pending.
