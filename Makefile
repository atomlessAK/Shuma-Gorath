...
SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE := $(call strip_wrapping_quotes,$(SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE))
...
SPIN_RUNTIME_CONTROL_ENV := --env SHUMA_ADMIN_CONFIG_WRITE_ENABLED=$(SHUMA_ADMIN_CONFIG_WRITE_ENABLED) --env SHUMA_POW_CONFIG_MUTABLE=$(SHUMA_POW_CONFIG_MUTABLE) --env SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE=$(SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE) --env SHUMA_BOTNESS_CONFIG_MUTABLE=$(SHUMA_BOTNESS_CONFIG_MUTABLE) --env SHUMA_DEBUG_HEADERS=$(SHUMA_DEBUG_HEADERS)
...
DEV_CHALLENGE_PUZZLE_CONFIG_MUTABLE ?= true
DEV_BOTNESS_CONFIG_MUTABLE ?= true
DEV_DEBUG_HEADERS ?= true
SPIN_DEV_OVERRIDES := --env SHUMA_POW_CONFIG_MUTABLE=$(DEV_POW_CONFIG_MUTABLE) --env SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE=$(DEV_CHALLENGE_PUZZLE_CONFIG_MUTABLE) --env SHUMA_BOTNESS_CONFIG_MUTABLE=$(DEV_BOTNESS_CONFIG_MUTABLE) --env SHUMA_DEBUG_HEADERS=$(DEV_DEBUG_HEADERS) --env SHUMA_ADMIN_CONFIG_WRITE_ENABLED=$(DEV_ADMIN_CONFIG_WRITE_ENABLED)
SPIN_PROD_OVERRIDES := --env SHUMA_DEBUG_HEADERS=false --env SHUMA_ADMIN_CONFIG_WRITE_ENABLED=false --env SHUMA_POW_CONFIG_MUTABLE=false --env SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE=false --env SHUMA_BOTNESS_CONFIG_MUTABLE=false
...
status: ## Check if Spin server is running
	@if curl -sf -H "X-Forwarded-For: 127.0.0.1" $(FORWARDED_SECRET_HEADER) $(HEALTH_SECRET_HEADER) http://127.0.0.1:3000/health > /dev/null 2>&1; then \
		...
		echo "   Maze Preview: http://127.0.0.1:3000/admin/maze/preview?path=%2Fmaze%2Fpreview (admin auth)"; \
	else \
...
env-help: ## Show supported env-only runtime overrides
	...
	@echo "  SHUMA_CHALLENGE_PUZZLE_CONFIG_MUTABLE"
