...
dev: ## Build and run with file watching (auto-rebuild on save)
	...
	@echo "$(CYAN)👀 Watching src/*.rs and spin.toml for changes... (Ctrl+C to stop)$(NC)"
	@echo "$(CYAN)ℹ️  Dashboard files are served directly; refresh the browser after CSS/JS/HTML edits.$(NC)"
	...
	@cargo watch -w src -w spin.toml -i '*.wasm' -i 'src/bot_trap.wasm' -i '.spin/**' \
...
test: ## Run ALL tests: unit tests first, then integration tests (requires server)
	...
	@cargo test || exit 1
	@echo ""
	@echo "$(CYAN)Step 2/2: Integration Tests (18 scenarios)$(NC)"
	@echo "$(CYAN)--------------------------------------------$(NC)"
...
test-integration: ## Run integration tests only (18 scenarios, requires running server)
